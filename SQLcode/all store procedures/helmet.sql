USE [TMMA_MAIN]
GO
/****** Object:  StoredProcedure [dbo].[helmet]    Script Date: 2021/3/5 上午 07:08:45 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
ALTER PROCEDURE [dbo].[helmet]
as
DECLARE  
@TotalNum INT, --執行次數
@Num INT,       --目前次數
...
@discount_membership numeric(10,6),   --20200210新增會員折扣
@discount_employee numeric(10,6),     --20200210新增員工折扣 
@discount_venter numeric(10,6)        --20200210新增經銷商折扣

select @TotalNum=count(MB001)  from INVMB 
left join POSMJ on POSMJ.MJ004=INVMB.MB001
where MB007='032' and MJ004 is null ;
...
set @CodeDate=substring(Convert(varchar(10),Getdate(),112),1,6)+ IIF(@snum+1 < 10,'00000'+convert(varchar(4),@snum+1),IIF((@snum+1 >=10 and @snum+1 <100),'0000'+convert(varchar(4),@snum+1),IIF((@snum+1 >=100 and @snum+1 <=999),'000'+convert(varchar(4),@snum+1),IIF((@snum+1 >=1000 and @snum+1 <=9999),'00'+convert(varchar(5),@snum+1),IIF((@snum+1 >=10000 and @snum+1 <=99999),'0'+convert(varchar(5),@snum+1),IIF((@snum+1 >=100000 and @snum+1 <=999999),convert(varchar(6),@snum+1),'XXX'))))));
   ...
   select top 1  @productID= MB001,
                 ...
                 from INVMB left join POSMJ on POSMJ.MJ004=INVMB.MB001 where MB007='032' and MJ004 is null ;
...
values
  ('TMMA_MAIN','DS','DS',@createdate,'','',1,@createtime,'','P001','POSI05','','','',0,'','DS',@eventcode,'4', @CodeDate,'頭盔促銷一般會員',@date1,@date2,@createdate,'2','1','','','','1',@createdate,'Y','DS',0,0,1,@discount_membership,'N','N','N','Y','N','','','','','',0,0,0,0,0)
...
MI018, MI019, MI020, MI021, MI022, MI023, MI024, MI025, UDF01, UDF02, UDF03, UDF04, UDF05, UDF06, UDF07, UDF08, UDF09, UDF10)
values
  ('TMMA_MAIN','DS','DS',@createdate,'','',1,@createtime,'','P001','POSI05','','','',0,'','DS',@eventcode,'4', @CodeDate,'頭盔促銷經銷商',@date1,@date2,@createdate,'2','2','','','','1',@createdate,'Y','DS',0,0,1,@discount_venter,'N','N','N','Y','N','','','','','',0,0,0,0,0)
...
MI018, MI019, MI020, MI021, MI022, MI023, MI024, MI025, UDF01, UDF02, UDF03, UDF04, UDF05, UDF06, UDF07, UDF08, UDF09, UDF10)
values
  ('TMMA_MAIN','DS','DS',@createdate,'','',1,@createtime,'','P001','POSI05','','','',0,'','DS',@eventcode,'4', @CodeDate,'頭盔促銷員工',@date1,@date2,@createdate,'2','3','','','','1',@createdate,'Y','DS',0,0,1,@discount_employee,'N','N','N','Y','N','','','','','',0,0,0,0,0)
...
MI018, MI019, MI020, MI021, MI022, MI023, MI024, MI025, UDF01, UDF02, UDF03, UDF04, UDF05, UDF06, UDF07, UDF08, UDF09, UDF10)
values
  ('TMMA_MAIN','DS','DS',@createdate,'','',1,@createtime,'','P001','POSI05','','','',0,'','DS',@eventcode,'4', @CodeDate,'頭盔促銷',@date1,@date2,@createdate,'2','','','','','1',@createdate,'Y','DS',0,0,1,1,'N','N','N','Y','N','','','','','',0,0,0,0,0)
