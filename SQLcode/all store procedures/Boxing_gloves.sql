USE [TMMA_MAIN]
GO
/****** Object:  StoredProcedure [dbo].[Boxing_gloves]    Script Date: 2021/3/5 上午 07:11:22 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
ALTER PROCEDURE [dbo].[Boxing_gloves]
as
...
where MB007='006' and MJ004 is null ;
...
set @Gift1='0103002400028'   -- 贈品品號
set @Gift2='0103002400035'   -- 贈品品號
set @Gift3='0102304000011'   -- 贈品品號
set @Gift4='9920400000041'   -- 贈品品號
set @createtime=  substring(convert(varchar(20),getdate(),113),11,9);
set @eventcode = '123'    -- 活動代號
...
set @CodeDate= substring(Convert(varchar(10),Getdate(),112),1,6)+ IIF(@snum+1 < 10,'00000'+convert(varchar(4),@snum+1),IIF((@snum+1 >=10 and @snum+1 <100),'0000'+convert(varchar(4),@snum+1),IIF((@snum+1 >=100 and @snum+1 <=999),'000'+convert(varchar(4),@snum+1),IIF((@snum+1 >=1000 and @snum+1 <=9999),'00'+convert(varchar(5),@snum+1),IIF((@snum+1 >=10000 and @snum+1 <=99999),'0'+convert(varchar(5),@snum+1),IIF((@snum+1 >=100000 and @snum+1 <=999999),convert(varchar(6),@snum+1),'XXX'))))));
   ...
   select top 1  @productID= MB001,
                 ...
                 @discount_membership= MB054,   --20200212新增
                 @discount_employee = MB055,    --20200212新增
                 @discount_venter = MB056       --20200212新增
                 from INVMB left join POSMJ on POSMJ.MJ004=INVMB.MB001 where MB007='006' and MJ004 is null ;
...
values
  ('TMMA_MAIN','DS','DS',@createdate,'','',1,@createtime,'','P001','POSI05','','','',0,'','DS',@eventcode,'4', @CodeDate,'拳套促銷一般會員',@date1,@date2,@createdate,'2','1','','','','1',@createdate,'Y','DS',0,0,1,@discount_membership,'N','N','N','Y','N','','','','','',0,0,0,0,0)
...
values
  (@createdate,@eventcode,'4', @CodeDate, @Gift1 ,'1',3,0,0,'0001')

insert into POSMK
  (CREATE_DATE, MK001, MK002, MK003, MK004, MK005, MK006, MK007, MK008, MK017)
values
  (@createdate,@eventcode,'4', @CodeDate, @Gift2 ,'1',3,0,0,'0002')

insert into POSMK
  (CREATE_DATE, MK001, MK002, MK003, MK004, MK005, MK006, MK007, MK008, MK017)
values
  (@createdate,@eventcode,'4', @CodeDate, @Gift3 ,'1',3,0,0,'0003')

insert into POSMK
  (CREATE_DATE, MK001, MK002, MK003, MK004, MK005, MK006, MK007, MK008, MK017)
values
  (@createdate,@eventcode,'4', @CodeDate, @Gift4 ,'1',3,0,0,'0004')
...
MI018, MI019, MI020, MI021, MI022, MI023, MI024, MI025, UDF01, UDF02, UDF03, UDF04, UDF05, UDF06, UDF07, UDF08, UDF09, UDF10)
values
  ('TMMA_MAIN','DS','DS',@createdate,'','',1,@createtime,'','P001','POSI05','','','',0,'','DS',@eventcode,'4', @CodeDate,'拳套促銷經銷商',@date1,@date2,@createdate,'2','2','','','','1',@createdate,'Y','DS',0,0,1,@discount_venter,'N','N','N','Y','N','','','','','',0,0,0,0,0)
...
MI018, MI019, MI020, MI021, MI022, MI023, MI024, MI025, UDF01, UDF02, UDF03, UDF04, UDF05, UDF06, UDF07, UDF08, UDF09, UDF10)
values
  ('TMMA_MAIN','DS','DS',@createdate,'','',1,@createtime,'','P001','POSI05','','','',0,'','DS',@eventcode,'4', @CodeDate,'拳套促銷員工',@date1,@date2,@createdate,'2','3','','','','1',@createdate,'Y','DS',0,0,1,@discount_employee,'N','N','N','Y','N','','','','','',0,0,0,0,0)
...
MI018, MI019, MI020, MI021, MI022, MI023, MI024, MI025, UDF01, UDF02, UDF03, UDF04, UDF05, UDF06, UDF07, UDF08, UDF09, UDF10)
values
  ('TMMA_MAIN','DS','DS',@createdate,'','',1,@createtime,'','P001','POSI05','','','',0,'','DS',@eventcode,'4', @CodeDate,'拳套促銷',@date1,@date2,@createdate,'2','','','','','1',@createdate,'Y','DS',0,0,1,1,'N','N','N','Y','N','','','','','',0,0,0,0,0)
